...
Для подачи retrieval-реплик в генеративную модель я установила порог в 0.85. Если топовая реплика имеют косинусную близость с контекстом + вопросом из диалога пользователя, то она не подается в генеративную модель. Если больше, то подается.
...
> [!IMPORTANT]
> Финальные метрики для генерации **temperature=1 и top_p=0.95**
