...
> [!IMPORTANT]
> Финальные метрики для генерации **temperature=0.95 и top_p=0.9** в попытке оставить разнообразие генераций и немного контроля над ними!
...
> [!IMPORTANT]
> Попробовать поговорить с Шелдоном можно по [ссылке](http://158.160.54.0:5000/). Это публичный IP ВМ на Yandex Cloud (ВМ: 60Гб размер дисков, 20Гб RAM, 2CPU). Работает **вроде бы быстро**, чтобы не стало скучно!
...
Хочу обратить внимание, что бесплатное размещение не включает GPU, только CPU, поэтому инференс работает медленнее, чем на локальном компьютере. Есть еще предположение, что hugging face замедляет использование асинхронных workers (описание ниже), при этом на платной ВМ от Yandex такого ограничения нет. На ВМ в Yandex также немного больше RAM. Кроме того, сервер на Hugging face работает в течение 48 часов после последнего посещения, поэтому при проверке может понадобиться еще раз запустить сервер.
